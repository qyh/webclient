...
function CMD.pem_request(session, url, get, post, cainfo, cert, sslkey)
    if get then
        local i = 0
        for k, v in pairs(get) do
            k = webclient:url_encoding(k)
    ...
    assert(cainfo and cert and sslkey, 'webclient:pem_request cert err!!')
    local req, key = webclient:pem_request(url, post, cainfo, cert, sslkey)
    if not req then
        return skynet.ret()
    end
    assert(key, 'webclient:pem_request key return nil!!')
...
function CMD.pem_get(session, url, form, cainfo, cert ,sslkey)
    return CMD.pem_request(session, url, form, nil, cainfo, cert, sslkey)
end

function CMD.pem_post(session, url, form, cainfo, cert, sslkey)
    return CMD.pem_request(session, url, nil, form, cainfo, cert, sslkey)
end
